...
mnist = input_data.read_data_sets(path, odd_even=True)
...
# Our modified version of MNIST is a binary classification problem: is the
# digit in a given image an odd or even number? To model this, we only need one
# output neuron. An output of 0 means even, 1 translates to odd.
# Because there is one weight for every neural connection, we would need 784x1
...
# for every neuron in the layer, 1 in the case of our output layer.
